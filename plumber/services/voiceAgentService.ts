...
// Swap-in alternative to Gemini Live. Model/voice IDs per OpenAI's GA
// Realtime API (2026) -- verify against https://platform.openai.com/docs
// if these have moved on since.
...
class VoiceAgentService {
  constructor() {
    this.provider = VOICE_PROVIDERS.OPENAI_REALTIME; // Default provider
    ...
    // OpenAI Realtime (WebRTC) state -- deliberately separate from the
    // Gemini fields above rather than reusing them, since only one
    // provider is ever active at a time but keeping them distinct makes
    // cleanup logic unambiguous.
    ...
    const savedOpenAIVoice = typeof window !== 'undefined' ? localStorage.getItem('joes_openai_voice') : '';
    const configuredOpenAIVoice = OPENAI_VOICES.some(v => v.id === savedOpenAIVoice) ? savedOpenAIVoice : 'marin';
    ...
    this.config = {
      ...
      // Which backend endpoint mints the OpenAI ephemeral token. Set this
      // per-site when instantiating (medspa vs plumber use different
      // routes, same as the Gemini WS paths below).
  ...
  configureProvider(providerName, config = {}) {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (config.openaiVoice) {
        localStorage.setItem('joes_openai_voice', config.openaiVoice);
      }
    }
  ...
  // ===========================================================================
  // OpenAI Realtime (WebRTC) provider -- swap-in alternative to Gemini Live.
  //
  // Key architectural difference from the Gemini path above: the browser
  // connects DIRECTLY to OpenAI over a WebRTC peer connection (not through
  // our own WebSocket proxy), using a short-lived token our backend mints.
  // Audio in both directions rides the WebRTC media track -- there is no
  // manual base64/PCM chunking, no scheduled AudioBufferSourceNodes, and no
  // hand-rolled jitter buffer. The browser's WebRTC stack (and OpenAI's)
  // handle jitter, packet loss, and playback timing for us.
  //
  // The data channel ("oai-events") carries only *events*: session config,
  // transcripts, tool calls, turn-taking signals. Not audio.
  // ===========================================================================
  ...
  async startOpenAIRealtimeSession() {
    ...
    try {
      // 1. Mint a short-lived client secret via our backend (never expose
      //    the real OPENAI_API_KEY to the browser).
      const tokenResponse = await fetch(this.config.openaiSessionEndpoint, { method: 'POST' });
      if (!tokenResponse.ok) {
      ...
      dc.onopen = () => {
        ...
        // Equivalent of the Gemini `setup` message: persona, voice, tools,
        // and transcription all configured from the client, same as today.
        this.sendOpenAIEvent({
          ...
          session: {
            ...
            audio: {
              ...
              output: {
                voice: this.config.openaiVoice || 'marin'
              }
            },
            tools: [{
              type: 'function',
      ...
      // 4. SDP offer/answer exchange directly against OpenAI's Realtime
      //    endpoint, authenticated with the ephemeral client secret (not
      //    our real API key). This is the step that makes this a genuine
      //    browser-to-OpenAI WebRTC connection rather than another relay.
      const offer = await pc.createOffer();
      await pc.setLocalDescription(offer);

      const sdpResponse = await fetch(
        `https://api.openai.com/v1/realtime/calls?model=${encodeURIComponent(model || this.config.openaiModel)}`,
        {
          method: 'POST',
          body: offer.sdp,
          headers: {
  ...
  // NOTE ON EVENT NAMES: these match OpenAI's GA Realtime server-event
  // schema as documented in 2026 (session.*, response.*, conversation.item.*,
  // input_audio_buffer.*). This is the part of the integration most likely
  // to drift as OpenAI revises the API -- if transcripts or tool calls stop
  // firing, check https://platform.openai.com/docs/api-reference/realtime-server-events
  // first for renamed/restructured events before debugging anything else.
  handleOpenAIServerEvent(msg) {
    switch (msg.type) {
      case 'input_audio_buffer.speech_started':
        // User started talking -- treat like Gemini's barge-in: whatever
        // Aura was saying gets cut off by WebRTC simply ceasing to send
        // more audio on the track server-side. No manual buffer to clear.
        ...
        // message appear before the AI starts responding.  The final
        // transcript will arrive shortly via
        // conversation.item.input_audio_transcription.completed and
        // will update this same bubble in place.
      ...
      case 'conversation.item.input_audio_transcription.completed':
        ...
        // If we already have a turnId from speech_started, update it in place
        // so the bubble doesn't duplicate. Otherwise create a fresh one.
      ...
      case 'response.audio_transcript.delta': // older event name, kept for safety
      ...
      default:
        // Plenty of other event types (response.output_item.added, etc.)
        // are informational only and safe to ignore here.
        break;
    }
  }
  ...
  async handleOpenAIFunctionCall(callId, name, argsJsonString) {
    ...
    // Prompt the model to continue speaking now that it has the tool result.
    this.sendOpenAIEvent({ type: 'response.create' });
  }
  ...
  startListening() {
    if (this.provider === VOICE_PROVIDERS.GEMINI_LIVE) {
      ...
      // OpenAI's server-side VAD (turn_detection) means the mic is always
      // being listened to once the session is connected -- there's no
      // separate "start listening" call the way Gemini's text-mode has.
